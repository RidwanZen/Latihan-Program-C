...
int main(int argc, char *argv[]){
	read_file("data_biner.txt");		// Membaca File "data_biner.txt" dengan memanggil fungsi read_file
	...
	for(int i=1;i<number_data+1;i++){		// Perulangan sebanyak number_data+1
		biner2dec (&datainput_1[i]);	// Memanggil function untuk mengubah bilangan biner menjadi decimal sebelum dikonvert ke string
		biner2dec (&datainput_2[i]);
		biner2dec (&datainput_3[i]);
		biner2dec (&datainput_4[i]);
		biner2dec (&datainput_5[i]);
	}
	...
	save_data_file("data_decimal.txt",dataBaca);	// Menyimpan data ke dalam file dengan memanggil Function
